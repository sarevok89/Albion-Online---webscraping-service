...
"""
Script fetching data from Albion Online killboard websites provided by user and generating
an output Excel file containing all the fetched data in a nice, user friendly manner.
"""
...
class ItemsUnique:
    ...
    This is all raw data e.g. main hand - "T6_MAIN_CURSEDSTAFF.png?count=1&quality=2".
    """
...
class Item:
    ...
    """
    Class, which takes a single ItemsUnique attribute, quality_dict and items_dict,
    to create detailed and user friendly output.
    ...
    In case there wasn't such item in our items_dict it sets it's name to 'Unknown'.
    """
...
def create_kill_id_list(text):
    ...
    """
    Function which takes user's input and using Regex module searches for Albion Online killboard link patterns.
    Even if user passess: "fsdhttps://albiononline.com/pl/killboard/kill/2490053454354", it is still going
    to get this "https://albiononline.com/en/killboard/kill/24900534" link. It returns a list with all URLs.

    :param text: Bunch of Albion Online killboard URLs (as mentioned above, they can be passed with some noise, e.g.:
    ...
    :return: A list of killboard IDs. From above example it would return a list like:
...
def create_table(kill_id_list):
    ...
    """
    Function that takes a list of all killboard URLs, returning a "dict_list" - list of dictionaries,
    containing all the user friendly data.

    :param kill_id_list: List returned by 'create_killboard_id_list' function.
    :return: A list of dictionaries. Each dictionary contains all fetched data from one URL, where keys are:
    ...
    def generate_dict(kill_id):
        ...
        """
        Function using BeautifulSoup, Selenium and Selenium Webdriver to fetch all the data from each killboard URL.
        It joins each 'kill_id' with 'base_url' to ensure that each url is formatted the same way and even if a user
        passes links from e.g. russian version of Albion Online page it is still going to work properly.

        'chrome_bin' is a variable provided by Heroku. To use Chromedriver with Heroku it's required to use their
        'heroku-buildpack-google-chrome' and 'heroku-buildpack-chromedriver'. Beside installing required webdriver and
        Chrome browser they also create environmental variables, which we later use in our script.

        :param kill_id: A single element from kill_id_list.
        :return: Single dictionary 'd' containing data fetched from one of 'kill_id_list' IDs.
        """
        ...
        WebDriverWait(browser, 10).until(lambda x: x.find_element_by_class_name('kill__body'))
        ...
        """
        We're opening a json file containing all Albion Online items we need. It contains computer friendly names
        with their user fiendly translations, e.g. 'T6_2H_DUALCROSSBOW_HELL': 'Boltcasters'.
        """
        ...
        """
        A simple dictionary used for translating items quality, e.g. in 'T6_2H_DUALAXE_KEEPER@3.png?count=1&quality=3'
    ...
    """
    We're looping through each element in 'kill_id_list' and applying our 'generate_dict' function. Then we append all
    the outcome to our 'dict_list' list.
    """
...
def generate_excel(dict_list, fight_name):
    """
    Function using dict_list to generate an Excel file from a Pandas DataFrame. It creates 'df' - Pandas DataFrame
    with specific, given column names and data from 'dict_list'. Each file name consists of:
    'day-month-year - fight_name.xlsx'

    :param dict_list: A list of dictionaries where each dictionary contains all fetched data from one URL.
    :param fight_name: Name of a fight recieved from user in our 'WebscraperForm'. It's being used in creating special
    name for each generated excel file.
    ...
    which in this case is Amazon S3.
    
    ExcelWriter doesn't allow us to directly create files on S3, so we temporarily create them on Heroku
    and upload to S3 right after that.
    
    We don't want our files to get overwritten in case user provided the same names for all the files
    generated on the same day, so each time we check, if a file with a name we want to provide already exists on
    S3 server. We consecutively add a number to a file's name and check again. We keep checking until
    we find a name that doesn't yet exist, e.g.: '13-2-2019 - onix vs squad(3).xlsx'.
    """
    ...
    """
    We adjust an Excel table to our needs. We remove header, make sure each cell has proper width
    and the text inside is centered.
    """
